...
class LiabilityReports():  
    ...
    def get_liability_accounts(self, user, details):
        ...
        try:            
            ...
            cur.execute("""SELECT sum(balance) totalliabilities FROM accounts WHERE status = %s AND balance > 0 AND (type = %s OR type = %s OR type = %s OR type = %s) ORDER BY id ASC""", (status, payable_accs, wallet_accs, longterm_loan_accs, shortterm_loan_accs))
            totalliability = cur.fetchone()

            total_liabilities = float(totalliability['totalliabilities'])


            cur.execute("""SELECT id, reference_no, number, name, balance FROM accounts WHERE status = %s AND balance > 0 AND (type = %s OR type = %s OR type = %s OR type = %s) ORDER BY id ASC""", (status, payable_accs, wallet_accs, longterm_loan_accs, shortterm_loan_accs))
