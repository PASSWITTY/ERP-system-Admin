...
class AssetsReports():    
    ...
    def get_assets_accounts(self, user, details):
        ...
        try:                        
            ...
            cur.execute("""SELECT sum(balance) totalassets FROM accounts WHERE balance > 0 AND status = %s AND (type = %s OR type = %s OR type = %s OR type = %s OR type = %s OR type = %s OR type = %s) ORDER BY type ASC""", [status, cash_accs, loan_assets_accs, inventory, receivable, other_current_asset_accs, fixed_asset_accs, other_asset_accs])
            totalasset = cur.fetchone()

            total_assets = float(totalasset['totalassets'])

            cur.execute("""SELECT id, reference_no, number, name, balance FROM accounts WHERE balance > 0 AND status = %s AND (type = %s OR type = %s OR type = %s OR type = %s OR type = %s OR type = %s OR type = %s) ORDER BY type ASC""", [status, cash_accs, loan_assets_accs, inventory, receivable, other_current_asset_accs, fixed_asset_accs, other_asset_accs])
            assets = cur.fetchall()
