...
class EquityReports(): 
    ...
    def get_equity_accounts(self, user, details):
        ...
        try:         
            ...
            cur.execute("""SELECT sum(balance) totalequity FROM accounts WHERE status = %s AND type = %s ORDER BY id ASC""", [status, equity])
            totalequity = cur.fetchone()

            total_equity = float(totalequity['totalequity'])

            cur.execute("""SELECT id, reference_no, number, name, balance FROM accounts WHERE status = %s AND type = %s ORDER BY id ASC""", [status, equity])
