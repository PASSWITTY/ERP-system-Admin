...
class ManagerDistribution():
    ...
    def list_stock_to_receive(self, user):
        ...
        try:
            ...
            cur.execute("""SELECT id, global_id, mobilephone_stock_received_id, distribution_center_id, manager_id, stockist_dispatch_date, manager_received_date, manager_remarks, stockist_remarks, stock_state, created_date, created_by FROM mobile_phones_manager_stock WHERE stock_state = %s AND status = %s """, (stock_state, status))
            ...
            if products_received:
                ...
                for transit in products_received:
                    ...
                    manager_id = transit["manager_id"]
                    stockist_dispatch_date = transit["stockist_dispatch_date"]
                    manager_received_date = transit["manager_received_date"]
                    distribution_center_id = transit["distribution_center_id"]
                    manager_remarks = transit["manager_remarks"]
                    stockist_remarks = transit["stockist_remarks"]
                    mobilephone_stock_received_id = transit["mobilephone_stock_received_id"]
                    ...
                    cur.execute("""SELECT id, first_name, last_name FROM user_details WHERE user_id = %s """, (manager_id))
                    ...
                    response = {
                        ...
                        "stockist_dispatch_date":stockist_dispatch_date,
                        "manager_received_date":manager_received_date,
                ...
                message = {'status':200,
                            ...
                            'description':'Manager stock to receive was fetched successfully!'
                        }   
                ...
                message = {'status':404,
                            'error':'sp_a04',
                            'description':'Failed to fetch Manager stock to receive!'
                        }   
        ...
        #Error handling
            ...
            message = {'status':501,
                       'error':'sp_a05',
                       'description':'Failed to retrieve Manager stock to receive from database.' + format(error)}
            ErrorLogger().logError(message),
    ...
    def list_stock_available(self, user):
        ...
        try:
            cur.execute("""SELECT id, global_id, mobilephone_stock_received_id, distribution_center_id, manager_id, stockist_dispatch_date, manager_received_date, stockist_remarks, manager_remarks, stock_state, created_date, created_by FROM mobile_phones_manager_stock WHERE stock_state = 1 AND status = 1 """)
            products_received = cur.fetchall()            
            if products_received:
                response_array = []
    ...
    def create_teamleader_dispatch(self, user):
        #Get the request data 
        request_data = request.get_json()       
    ...
    def list_teamleader_dispatched_stock(self, user):
        ...
        try:
            ...
            if phone_devices:
                ...
                for phone_device in phone_devices:
                    ...
                    manager_details = cur.fetchone()            
                    if manager_details:
                        first_name = manager_details["first_name"]
                        last_name = manager_details["last_name"]
    ...
    def approve_teamleader_dispatched_stock(self, user):
        request_data = request.get_json() 
        ...
        try:  
            ...
            cur.execute("""SELECT mobilephone_managerstock_id FROM mobile_phones_teamleader_stock WHERE status = 2 AND id = %s""", [id])
            ...
            #stock_state = 2 means manager has disbursed their stock to team leaders
